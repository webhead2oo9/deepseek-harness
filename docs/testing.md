...
- **Real-API e2e** (`pnpm run test:e2e`): explicit tests against live provider APIs; suites gate on their own keys (`DEEPSEEK_API_KEY`, `EXA_API_KEY`, `PERPLEXITY_API_KEY`, …), self-skip without them, and never run in GitHub Actions ([real-API e2e Agent Note](../.agents/notes/implemented/testing/2026-08-18-remove-real-api-ci-workflow.md)).
