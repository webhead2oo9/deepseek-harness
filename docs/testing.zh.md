...
- **真实 API e2e**（`pnpm run test:e2e`）：显式测试会访问真实提供方 API；套件各自由自己的密钥控制（`DEEPSEEK_API_KEY`、`EXA_API_KEY`、`PERPLEXITY_API_KEY` 等），缺少密钥时自动跳过，且绝不在 GitHub Actions 中运行（[真实 API e2e Agent Note](../.agents/notes/implemented/testing/2026-08-18-remove-real-api-ci-workflow.md)）。
