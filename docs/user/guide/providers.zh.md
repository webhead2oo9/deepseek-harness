...
打开**设置 → 子代理**，为委派任务创建具名的提供商/模型配置。每个配置包含简短名称、告诉父 Agent 何时使用它的说明、确切的提供商/模型路由，以及每次选择该配置时应用的可选多行子代理系统指令。提供商和模型字段会给出实时目录建议，也接受私有 RunInfra 路由等手动值。

具名配置会成为兼容 `subagent` 和 `subagent_fork` 工具的可选 `profile`。只有在允许父 Agent 选择任意提供商/模型对时才开启**允许直接选择模型**；该开关默认关闭。由外部产品自身管理模型配置的固定子代理后端不会公开这两类选择器。

配置修改只影响未来的委派。已有的可继续子代理会保留创建时使用的提供商/模型路由和子代理系统指令，即使对应配置随后被修改或删除。
