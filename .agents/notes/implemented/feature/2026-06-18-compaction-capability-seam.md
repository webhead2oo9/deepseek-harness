...
`compactIfNeeded(agent, trigger, signal)` takes either exact imminent-pressure facts or a provider-confirmed context-overflow trigger plus cancellation. Pressure carries the canonical request header and active capacity; overflow reads the latest durable route. Both use the singleton estimator. `compactNow(agent, signal)` requires an idle agent and performs one useful balanced reduction even below pressure, returning `null` without writes when none exists. `compactRegion(start, end, agent, signal?)` uses `agent.session` as its single session identity and keeps an optional signal for explicit callers. The default summarizer resolves its target from explicit config, the latest logged routed target, then agent options, and records the provider/model pair after any `llm/stream` routing. It replays the routed request's prefix and appends the compaction directive as a trailing user message so the provider's warm KV cache is reused — see the [summary prefix-cache Agent Note](../bug-fix/2026-07-21-compaction-summary-prefix-cache-reuse.md). The result carries `llmStreamCall: true` because it consumed exactly one call through this context's LLM service; a subclass sets that marker only under the same condition, since retained `rawOutput` alone does not identify the call path. The call sets the provider-neutral `GenerateOptions.purpose` to `compaction`; adapters may map that purpose to model-hidden transport metadata, and the DeepSeek adapter sends `x-deepseek-harness-compact: 1`.

### Automatic pressure admits the complete immutable request

Pressure runs at `agent/request-admission`, after entered messages, final routing, system prompt, tools, adapter defaults, and active capacity are fixed and before adapter dispatch. `dsh-compaction-basic` measures that canonical request and current durable surface through `ctx.tokenMeter`. Once pressure qualifies, optional `ctx.toolResultPruner` rewriting runs before summary selection; compaction-basic remeasures the durable surface and skips summarization if pruning restores safe pressure. Any durable replacement makes the loop rebuild messages and redispatch admission without repeating request middleware or adapter preparation. The [complete-request admission decision](../bug-fix/2026-08-17-complete-request-compaction-admission.md) owns this lifecycle.
...
```
claim and record entered messages → prepare immutable request
await waterfall agent/request-admission  ⟵ pressure compaction before adapter dispatch
replacement → rebuild messages → admission
...
Auto-compaction checks **every complete imminent request**, not once per turn. This is load-bearing for runaway-turn survival: a tool-heavy ReAct turn appends an `assistant/message` + a `tool/result` per step, so the surface grows within a turn. Admission can compact early closed tool pairs after the continuation step has recorded its entering messages but before its model dispatch; provider-confirmed overflow remains the backstop for unrecognized or indivisible pressure.
...
- **Compaction on `agent/request` or a compaction-specific loop callback** — rejected because the former observes a provisional request and the latter couples generic lifecycle to one policy. The generic `agent/request-admission` extension accepts or rebuilds any immutable request, while the compaction plugin owns its pressure decision.
...
- **Automatic extension points**: `agent/request-admission` (`@mode waterfall`) handles pressure after immutable request construction and before adapter dispatch; `agent/request-error` (`@mode waterfall`) handles final request failures after the failed step closes. Admission carries the frozen request, active capacity, rebuild count, turn, step, and signal (see the [payload-object events decision](../architecture/2026-08-06-agent-event-payload-objects.md)), with no compaction-only prompt/prefix payload.
...
- **Loop:** Tests pin admission after entered-message and request construction but before adapter dispatch, actual `agent/request` routing and capacity, generation-proven message rebuild, closed failed steps, fresh retry numbering, and complete thrown/in-band overflow → compaction → reconstructed retry composition.
