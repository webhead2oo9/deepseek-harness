...
适配器从已认证的 `/v1/language-models` 端点发现可聊天条目，并通过共享的 pi-ai OpenAI Responses 传输提供这些模型。发现期间收到 401 时会关闭响应、强制执行一次串行刷新并重试一次。成功目录按配置时长缓存。未登录路由保留一个小型静态种子，使认证与模型界面始终可寻址；认证后的发现会替换它。静态 pi-ai 元数据提供已知模型的容量、模态和推理等级；新 ID 使用明确且可配置的容量默认值，不推断推理控制。
...
包测试覆盖挑战就绪、取消结算、记录校验、到期刷新、未登录目录可见性、已认证发现、一次性 401 刷新、畸形响应、端点限制、插件注册和 invariant 所有权。组装后的 Web snapshot 覆盖新的账户认证卡；从源码启动的 Web 实例验证真实设备登录、在线目录和 Responses 请求路径。
...
个人 xAI 订阅账户可以在没有 API key 的情况下认证，并选择其账户可访问的模型。资格仍由提供方控制，因此完成浏览器步骤并不保证订阅等级允许 API 请求。新模型 ID 可立即以保守容量使用，但只有在 pi-ai 编目或部署配置扩展后才会获得更丰富的推理元数据。
