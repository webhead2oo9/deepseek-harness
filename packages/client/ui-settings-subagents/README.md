...
This browser Cordis plugin registers the `subagents` settings section. It edits the Host namespace `subagent-model-selection`, whose value contains `allowDirectModelSelection` and a profile dictionary. Each profile has a description, provider, model, and optional multiline child system instruction and adapter-owned reasoning-effort id. The editor omits blank optional fields on user-owned profiles; for a deployment default, blank instruction or effort persists a `null` reset marker so the composition value does not reappear through recursive settings merging. Cards show only effective configured instruction and effort values. Profile names and stored fields are preserved exactly; model-adapter data is advisory and does not restrict manual route or effort strings. The exact child route validates an effort when the profile is used.
