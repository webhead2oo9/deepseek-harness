...
A child session whose own scope composes a different request declares it per fixture index: `pinsChildToolSchemas` moves that child's tool sequence into `tool-schemas.<n>.expected.json`, `pinsChildSystemPrompts` moves its prompt into `system-prompt.<n>.expected.md`, and `pinsChildRequestConfigs` retains its route and request options in `session.<n>.jsonl`. Each names the child fixture it describes; undeclared request-header fields still come from the class pin, and sidecars exist exactly when declared. A child prompt sidecar must also differ from its class pin, so a redundant copy fails instead of drifting. Continuable report guidance exercises the prompt/schema pins, while configured subagent profiles exercise the request-config pin.
