...
- **Measurement** — immediately before adapter dispatch, the singleton `ctx.tokenMeter` prices the complete immutable request header and current durable surface at one consumed-log revision. Pressure therefore includes newly entered messages, the actual system prompt, tools, routing, assistant completion, tool results, buffered context, and steering.
- **Routed policy** — proactive pressure receives the active capacity from the adapter registration that prepared the imminent request, then scales the default policy plus an optional exact-target override into concrete token budgets. Model discovery remains advisory and is not consulted separately.
...
| `auto` | no (default `true`) | Register complete-request admission and overflow-recovery listeners. Set `false` for manual-only. |
...
When a complete imminent request crosses the threshold, oversized tool results are first rewritten if the optional pruner is loaded. If summarization remains necessary, that request is rebuilt before dispatch and receives the checkpoint preamble below, a blank line, `<compacted-summary>`, the data-dependent summary, and `</compacted-summary>`. Overflow recovery rebuilds the immediate retry from whatever replacement advanced the surface. A checkpoint replaces the selected older range and is followed by the retained recent units.
...
- **Overflow classification is adapter-maintained** — provider wording can change; the DeepSeek adapters and native Codex adapter normalize currently recognized context-limit failures to `CONTEXT_WINDOW_EXCEEDED`.
