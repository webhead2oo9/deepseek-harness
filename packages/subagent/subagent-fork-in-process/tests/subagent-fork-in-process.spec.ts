...
describe('dsh-subagent-fork-in-process', () => {
  ...
  it('advertises every start-time capability (depthLimit, outputSchema, toolFilter, persona, instruction, reasoningEffort, modelRoute)', async () => {
    const { ctx } = await setup([])
    expect(ctx.subagents.getProvider('fork')!.capabilities).toEqual({ outputSchema: true, depthLimit: true, toolFilter: true, persona: true, instruction: true, reasoningEffort: true, modelRoute: true })
  })
