...
`baseURL` 默认为 `https://api.x.ai/v1`。生产配置只接受使用 HTTPS 的 `x.ai` 主机，防止已存储的 bearer 令牌被重定向到其他来源；测试仍可使用 HTTP 回环端点。发现的模型会尽可能继承 pi-ai 静态目录中的容量与推理元数据；较新的模型 ID 使用可配置的保守回退值。
...
- 新发现的模型 ID 使用配置的容量回退值，并且在 pi-ai 发布匹配元数据前不提供推理选择器。
